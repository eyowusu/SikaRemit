...
const kycService = {
  ...
  checkEligibility: async (): Promise<{
    ...
    try {
      const response = await api.get(ENDPOINTS.KYC.ELIGIBILITY);
      return response.data;
    } catch (error: any) {
      return {
        eligible: true,
  ...
  startVerification: async (): Promise<KYCVerification> => {
    const response = await api.post(ENDPOINTS.KYC.VERIFICATION, {
      action: 'start',
    });
    return response.data;
  },
  ...
  uploadDocument: async (request: DocumentUploadRequest): Promise<KYCVerification> => {
    ...
    const response = await api.post(ENDPOINTS.KYC.DOCUMENTS, {
      type: request.type,
  ...
  uploadSelfie: async (request: SelfieUploadRequest): Promise<KYCVerification> => {
    ...
    const response = await api.post(ENDPOINTS.KYC.BIOMETRICS, {
  ...
  performLivenessCheck: async (imageUri: string): Promise<{
    ...
    try {
      ...
      const response = await api.post(ENDPOINTS.USER.CHECK_LIVENESS.replace('{id}', 'me'), {
        image: imageBase64,
      });
