...
class PaymentServiceTests(TestCase):
    ...
    @patch('payments.services.payment_processing_service.PaymentService._perform_fraud_analysis')
    def test_fraud_analysis_integration(self, mock_fraud):
        """Test fraud analysis is called during payment processing"""
        from payments.services.payment_processing_service import PaymentService
...
class CardPaymentTests(TestCase):
    ...
    def test_process_card_payment_success(self, mock_intent, mock_account):
...
class PayoutTests(TestCase):
    ...
    @patch('requests.post')
    def test_bank_payout_success(self, mock_post):
        """Test successful bank payout"""
        from payments.services.payment_processing_service import PaymentService
        
        mock_post.return_value = MagicMock(
            status_code=200,
            json=lambda: {'data': {'id': 'transfer_123'}},
            text='{"data": {"id": "transfer_123"}}'
        )
...
class KYCEligibilityTests(TestCase):
    ...
    def test_kyc_eligible_user(self, mock_get):
